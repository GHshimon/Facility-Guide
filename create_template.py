...
col_headers = [
    "ID", "施設名", "施設種別",
    "自立1", "自立2", "要介護1", "要介護2", "要介護3", "要介護4", "要介護5",
    "個室", "多床室",
    "インスリン", "胃瘻", "経管栄養", "喀痰吸引",
    "バルーン", "褥瘡処置", "透析",
    "生保受入", "第1段階", "第2段階", "第3-1段階", "第3-2段階", "第4段階",
    "エリア", "住所", "電話", "FAX", "担当者名", "担当者メモ",
    "相談員柔軟性", "空床傾向", "判定会議頻度",
    "施設の強み", "月額費用目安", "備考（生情報）", "検索情報",
]
...
LEFT_ALIGN_COLS = {2, 27, 28, 29, 30, 31, 32, 35, 36, 37, 38}
...
for r_idx, row in enumerate(sample, start=4):
    ...
    for c_idx, val in enumerate(row, start=1):
        ...
        c.font = BODY_FONT
        c.border = BORDER
        c.alignment = CENTER if c_idx not in LEFT_ALIGN_COLS else LEFT
        ox_fill = cell_fill_for_ox(val) if isinstance(val, str) else None
        c.fill = ox_fill if ox_fill else fill
    ws.row_dimensions[r_idx].height = 18

ws.merge_cells(f"A203:{LAST_COL_LETTER}203")
ws["A203"] = "※ 医療ケア欄は「○」「×」「要相談」。自立・要介護度・部屋・生保・負担段階は「○」「×」。"
...
guide = [
    ...
    ("検索情報", "公的データの出典・事業所番号など（任意）", "事業所番号:…"),
]
